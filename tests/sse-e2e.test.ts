...
describe('end-to-end SSE contract_event push', () => {
  ...
  it('subscribed client receives contract_event notification and unsubscribed does not', async () => {
    ...
    try {
      await clientA.callTool({ name: 'register_agent', arguments: { agent_type: 'custom', name: 'tester-11', model: 'm', role: 'r' } })
      await clientB.callTool({ name: 'register_agent', arguments: { agent_type: 'custom', name: 'tester-12', model: 'm', role: 'r' } })
      await clientC.callTool({ name: 'register_agent', arguments: { agent_type: 'custom', name: 'tester-13', model: 'm', role: 'r' } })
