...
describe('register_agent tmux handling integration', () => {
  ...
  it('list_agents returns tmux_pane_id: null when registration does not find a pane', async () => {
    ...
    const regA = JSON.parse(((await A.c.callTool({ name: 'register_agent', arguments: { model: 'opus-4-7', role: 'frontend', name: 'alice' } })).content as Array<{ text: string }>)[0].text) as { agent_id: string }
