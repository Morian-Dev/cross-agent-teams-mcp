...
describe('register_agent name field validation', () => {
  ...
  it('rejects when name is missing', async () => {
    ...
    const resp = await c.callTool({ name: 'register_agent', arguments: { model: 'opus', role: 'backend' } }) as { isError?: boolean; content: Array<{ text: string }> }
  ...
  it('rejects when name is whitespace only', async () => {
    ...
    const { c, t } = await connect(b.host, b.port)
    teardown.push(async () => { await t.close(); await c.close() })
    const resp = await c.callTool({ name: 'register_agent', arguments: { model: 'opus', role: 'backend', name: '   ' } }) as { isError?: boolean; content: Array<{ text: string }> }
    expect(resp.isError).toBe(true)
  ...
  it('role defaults to "default" when omitted', async () => {
    ...
    const { c, t } = await connect(b.host, b.port)
    teardown.push(async () => { await t.close(); await c.close() })
    const resp = await c.callTool({ name: 'register_agent', arguments: { model: 'opus', name: 'alice', tmux_pane_id: '%1' } })
  ...
  it('team defaults to "default" when omitted', async () => {
    ...
    const { c, t } = await connect(b.host, b.port)
    teardown.push(async () => { await t.close(); await c.close() })
    const resp = await c.callTool({ name: 'register_agent', arguments: { model: 'opus', name: 'alice', role: 'backend', tmux_pane_id: '%1' } })
