...
describe('poke tmux_cmd_failed with stage info', () => {
  ...
  it('returns tmux_cmd_failed with stage "load_buffer" when loadBuffer rejects unexpectedly', async () => {
    ...
    db.prepare(`INSERT INTO agents (agent_id, team, role, display_name, model, registered_at, last_seen_at, tmux_pane_id) VALUES (?,?,?,?,?,?,?,?)`).run(
      'caller-a', 'default', 'dev', null, 'opus-4-7', new Date().toISOString(), new Date().toISOString(), null
    )
    db.prepare(`INSERT INTO agents (agent_id, team, role, display_name, model, registered_at, last_seen_at, tmux_pane_id) VALUES (?,?,?,?,?,?,?,?)`).run(
      'target-b', 'default', 'dev', null, 'gpt-5', new Date().toISOString(), new Date().toISOString(), '%42'
    )
