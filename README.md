...
**Sessions accumulate, permanently.**  The launcher creates a *new* session on every invocation — that is the point, since the exact `session_id` is what makes pokes land on the session you are actually looking at.  But kimi's REST API has no route for deleting a session: its whole surface exposes three `DELETE` endpoints and none of them is for sessions, and `DELETE /api/v1/workspaces/{id}` only unregisters a workspace ("does not remove on-disk content") while leaving its sessions listed.  So every `xats-kimi` run leaves a session behind for good, and the only way to prune them is the kimi TUI or `kimi web` UI.  If you launch often, expect `~/.kimi-code/sessions/` to grow and clear it out periodically.
