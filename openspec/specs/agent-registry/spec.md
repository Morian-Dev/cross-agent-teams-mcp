...
The daemon SHALL expose a tool `register_codex_self` for Codex remote sessions.  The tool accepts human-facing identity fields such as `name`, `team`, and `role`, plus optional `ws_url` and `auth_token_ref`.  It SHALL:
...
#### Scenario: register_codex_self registers the single resumable thread

- **GIVEN** the caller invokes `register_codex_self({ name: 'lead', team: 'default', role: 'worker' })`
