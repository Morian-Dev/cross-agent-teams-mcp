...
**Reason**: Replaced by `register_agent reuses agent_id by (team, name, role) identity`. The session-id-as-agent-id model conflated transport and identity; reconnects produced new agent_ids and broke mailbox continuity. Identity is now derived from `(team, name, role)` with `agent_id` as a stable UUID.
**Migration**: Fresh-boot only. Users must drop `~/.agent-teams/daemon.db` (or configured path) and re-register. MVP does not provide legacy migration.

### Requirement: Repeated register_agent within same session updates metadata

**Reason**: Superseded by `Repeated register_agent for same identity updates metadata`, which generalises upsert semantics from "same session" to "same `(team, name, role)` identity regardless of session".
**Migration**: None required on a fresh boot.

### Requirement: agent_id collision across sessions returns 409

**Reason**: Replaced by `Within-session agent_id_collision via Authorization header`. The old "across sessions" phrasing conflicts with cross-session identity reuse — legitimate reconnects would have been blocked. Collision protection is now scoped strictly to within-session Authorization mismatches.
**Migration**: None required on a fresh boot.

### Requirement: Mismatched agent_id for session returns 403

**Reason**: Replaced by `Mismatched agent_id for tool call returns 403`, which checks against the session's currently registered `agent_id` (held in `agentIdHolder.current`) rather than the MCP session id itself. This is necessary because `agent_id` is no longer the session id.
**Migration**: None required on a fresh boot.
