...
**Migration**: `stop-server.sh` wipes `data.db` on shutdown. Post-change daemon bootstrap produces a fresh schema without these columns. Agents bound to opencode hosts carry only `tmux_pane_id` (populated via pid → tty → pane from `register_agent({agent_type:'opencode', ui_pid})`).
...
**Migration**: The replacement flow is `tmux new-window; opencode; register_agent({agent_type:'opencode', ui_pid:<pid>})`. No pre-reg call is required; the daemon binds the pane directly from `ui_pid`.
...
**Migration**: Callers invoke `register_agent({agent_type:'opencode', name, model, ui_pid, project_dir, ...})` instead. The daemon's generic pid-based pane binding path populates `tmux_pane_id`; no opencode-specific metadata is written.
...
### Requirement: register_agent agent_type=opencode consumes pre-reg when opencode metadata is omitted
**Reason**: Pre-reg consumption is being removed along with the pre-reg table and the opencode transport columns. The opencode branch of `register_agent` reduces to the same path as `agent_type:'custom'` plus the label preservation.
**Migration**: `register_agent({agent_type:'opencode', ui_pid, ...})` binds only `tmux_pane_id` via the generic pid-based path. No `base_url` / `session_id` fields are accepted or persisted.
...
**Migration**: Callers use `register_agent({agent_type:'opencode', ui_pid, ...})` directly. `register_agent`'s description SHALL NOT mention `register_opencode_self`, `pre_register_opencode_pane`, `bind_opencode_session`, `base_url`, or `session_id` as opencode-specific concerns.
