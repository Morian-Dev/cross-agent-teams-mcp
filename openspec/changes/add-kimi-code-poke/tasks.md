...
- [x] 6.8 Migrate to kimi 0.28.0 server commands: `kimi server run` deprecated → all launcher/lifecycle commands switched to `kimi web --no-open` / `kimi web kill`; the 0.27-era 60s idle-exit is gone so no `--keep-alive` needed; shell functions moved from `~/.zshrc` to `~/.xats.sh` (sourced); verified live on 0.28.0 (server_version via /api/v1/meta, token file unchanged, REST paths unchanged, pokes still wake sessions)
