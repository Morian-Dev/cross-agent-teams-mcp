...
- [ ] 2.1 Define a public delivery projection type (e.g. `PublicDelivery = {kind: 'none' | 'codex-appserver'} | {kind: 'claude-channel', channel_session_id: string}`) and a `toPublicAgentRow` helper colocated with the MCP layer (new file `src/mcp/agent-public-row.ts` OR inline in `src/mcp/tools.ts` — pick one in apply)
- [ ] 2.2 Update `list_agents` handler in `src/mcp/tools.ts` to map each `AgentListRow` through the projection before returning
- [ ] 2.3 Keep the top-level `channel_session_id: string | null` field in the projected row unchanged (derived from `delivery.kind === 'claude-channel'`)
- [ ] 2.4 Confirm internal repo API (`AgentsRepo.getById`, `AgentsRepo.list`) still returns the full `DeliverySpec` — do not narrow these
- [ ] 2.5 Update `tests/agents-repo-list-channel-session-id.test.ts` to assert the projection rules
- [ ] 2.6 Add a test (new file or existing `list_agents` test) seeding a codex-appserver agent directly via `AgentsRepo.setDelivery`, calling `list_agents`, and asserting `delivery.kind === 'codex-appserver'`, absence of `delivery.thread_id`, `delivery.ws_url`, `delivery.auth_token_ref`
- [ ] 2.7 Audit other `list_agents` tests and any test that asserts on `delivery` shape to confirm they still pass or update them to match the projected shape
