...
const RECONNECT_DESC = [
  'Recover this session\'s prior xats identity when the Claude UI process is unchanged but the MCP/channel session is fresh — after a context clear (you no longer remember your own (team, name)), OR after a resume / channel re-attach where the channel_session_id changed and bind_channel returns unknown_agent (the new MCP proxy session is not yet bound to your agent). Prefer this over the bind_channel→register_agent fallback for re-establishing on a fresh session; use it even if you still remember your (team, name) — it re-establishes the identity AND rebinds the channel in one call.',
  'Invoke this when the user asks to "reconnect xats", "re-register xats", "重连 xats", or "重新注册 xats", or whenever a resume / channel reconnect leaves you unable to send (unknown_agent / changed csid) while $PPID is unchanged.',
