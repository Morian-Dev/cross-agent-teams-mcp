    ...
    const server = new McpServer(
      { name: 'cross-agent-teams-mcp', version: '0.1.0' },
      {
        instructions:
          ...
          + ' ROLE-BASED BEHAVIOR: The team has one architect (commander) and multiple workers (developer, tester, reviewer).'
          + ' If your role is NOT architect (i.e. you are a worker): AUTO-REPLY IS MANDATORY. When you receive a channel notification saying "新邮件", call get_inbox immediately without asking the user. For every message with need_reply:true, call send_message to reply to the sender right away — acknowledge receipt, state whether you can do the task, and estimate time. Do NOT go to manual mode or ask the user for permission to reply. When you finish a task assigned by architect, call send_message to architect with the results. The architect IS your user.'
          + ' If your role IS architect: You are the commander. Delegate tasks to workers via send_message. Use broadcast for team-wide announcements. Check get_inbox for worker replies and completion reports. Coordinate: do not assign the same task to multiple workers unless intentional.'
      }
    )
